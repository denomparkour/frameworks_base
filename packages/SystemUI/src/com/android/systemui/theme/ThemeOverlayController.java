...
public class ThemeOverlayController extends SystemUI {
    ...
    static final String OVERLAY_ATOM_BLACK_THEME =
            "com.atom.overlay.android.blacktheme";
    ...
    private void applyBlackTheme(boolean state) {
        ...
        try {
            mOverlayManager.setEnabled(OVERLAY_ATOM_BLACK_THEME, state, userId);
            if (DEBUG) {
                Log.d(TAG, "applyBlackTheme: overlayPackage="
                        + OVERLAY_ATOM_BLACK_THEME + " userId=" + userId);
            }
        } catch (Exception e) {
            Log.e(TAG, "Failed to " + (state ? "enable" : "disable")
                    + " overlay " + OVERLAY_ATOM_BLACK_THEME + " for user " + userId);
        }
    }
